...
const _send = ({ subject, emails, message }, cb = null) => {
    const mailOptions = { 
        from: "no-response@everestdevelopmentmaster.com",
        to: emails,
        subject,
        html: message
    };
...
const enviarNovoPedido = ({ usuario, pedido }) => {
    const message = `
        ...
        <br /><br />
        <p>Atenciosamente,</p>
        <p>Equipe - Everest Sistemas</p>
    `;
    _send({
        subject: "Pedido Recebido - Everest Sistemas",
...
const cancelarPedido = ({ usuario, pedido }) => {
    ...
    `;
    _send({
        subject: "Pedido Cancelado - Everest Sistemas",
        emails: usuario.email,
        message
    });
}
...
const atualizarPedido = ({ usuario, pedido, status, data, tipo }) => {
    ...
    `;
    _send({
        subject: "Pedido Atualizado - Everest Sistemas",
        emails: usuario.email,
        message
    });
}
