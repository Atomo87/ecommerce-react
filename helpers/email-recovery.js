...
const smtpTransport =  mailer.createTransport(require('../config/email'));
...
module.exports = ({ usuario, recovery }, cb) => {
    const message = `
        ...
        <br />
        <p>Atenciosamente, Everest Sistemas</p>
    `;

    const opcoesEmail = {
        from: "everestdevelopmentmaster@gmail.com",
        to: usuario.email,
        subject: "Redefinicao de Senha - Everest Sistemas",
        html: message
    };

    if( process.env.NODE_ENV !== "production" ){  // mudar quando for producao
        smtpTransport.sendMail(opcoesEmail, (error, info) => {
            if(error){
                console.log(error);
